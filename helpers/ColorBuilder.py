...
class ColorBuilder:
    ...
    def get_color_box(self, segments):
        ...
        if self.primary_color_box is not None:
            return self.primary_color_box

        point = self.primary_color_coordinate
        height = self.height
        width = self.width

        # For easier calculations, if it's on the edge, nudge it off
        if point[0] == width:
            point[0] -= 1
        if point[1] == height:
            point[1] -= 1
        ...
        # Filter through all horizontal segments
        horiz_segs = segments['y']

        # all segments north of test point
        north_segs = [seg for seg in horiz_segs if seg[1][1] < point[1]]
        # includes edges
        north_segs.append([[0, 0], [width, 0]])
        # all north segments that intersect with the point's x coordinates
        north_segs_intersect = [seg for seg in north_segs if between(point, seg, 0)]
        # the closest northern segment
        north_segs_closest = max(north_segs_intersect, key=lambda seg: seg[0][1])

        south_segs = [seg for seg in horiz_segs if seg[1][1] >= point[1]]
        south_segs.append([[0, height], [width, height]])
        south_segs_intersect = [seg for seg in south_segs if between(point, seg, 0)]
        south_segs_closest = min(south_segs_intersect, key=lambda seg: seg[0][1])


        # Repeat process for vertical segments
        vert_segs = segments['x']

        west_segs = [seg for seg in vert_segs if seg[1][0] < point[0]]
        west_segs.append([[0, 0], [0, height]])
        west_segs_intersect = [seg for seg in west_segs if between(point, seg, 1)]
        west_segs_closest = max(west_segs_intersect, key=lambda seg: seg[0][0])

        east_segs = [seg for seg in vert_segs if seg[1][0] >= point[0]]
        east_segs.append([[width, 0], [width, height]])
        east_segs_intersect = [seg for seg in east_segs if between(point, seg, 1)]
        ...
        x1 = max([p[0] for p in rel_points if p[0] <= point[0]])
        x2 = min([p[0] for p in rel_points if p[0] > point[0]])
        y1 = max([p[1] for p in rel_points if p[1] <= point[1]])
        y2 = min([p[1] for p in rel_points if p[1] > point[1]])
